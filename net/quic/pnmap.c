...
#define QUIC_PN_MAP_RTT 2000 /* skip the old packet numbers every RTT */
...
struct quic_pnmap *quic_pnmap_init(struct quic_pnmap *map)
{
	...
	map->max_pn_seen = map->cumulative_pn_ack_point;
	map->last_ts = jiffies_to_msecs(jiffies);
	map->last_max_pn = 0;
...
int quic_pnmap_mark(struct quic_pnmap *map, u32 pn)
{
	...
	if (pn == map->cumulative_pn_ack_point + 1) {
		...
		if (quic_pnmap_has_gap(map)) {
			...
			map->cumulative_pn_ack_point = map->base_pn + zero_bit;
		}
	}
...
static void quic_pnmap_update(struct quic_pnmap *map)
{
	u32 current_ts = jiffies_to_msecs(jiffies);
	unsigned long zero_bit, len;

	if (current_ts - map->last_ts < QUIC_PN_MAP_RTT)
		return;

	len = map->last_max_pn - map->base_pn;
	map->base_pn = map->last_max_pn;

	map->last_ts = current_ts;
	map->last_max_pn = map->max_pn_seen;

	if (len) {
		bitmap_shift_right(map->pn_map, map->pn_map, len, map->len);
		len = map->last_max_pn - map->base_pn + 1;
...
static int quic_pnmap_grow(struct quic_pnmap *map, u16 size)
{
	...
	bitmap_copy(new, map->pn_map, map->max_pn_seen - map->cumulative_pn_ack_point);
	kfree(map->pn_map);
	map->pn_map = new;
	map->len = len;

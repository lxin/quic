...
static int quic_v6_flow_route(struct sock *sk)
{
...
static int quic_v4_flow_route(struct sock *sk)
{
	struct quic_sock *qs = quic_sk(sk);
	union quic_addr *addr;
	struct flowi4 *fl4;
...
int quic_flow_route(struct sock *sk)
{
	return quic_sk(sk)->af_ops->flow_route(sk);
}

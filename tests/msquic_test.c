...
void PrintUsage()
{
    printf(
        ...
        "Usage:\n"
        "\n"
        "  ./msquic_test -client -unsecure -target:{IPAddress|Hostname} [-ticket:<ticket>]\n"
        "  ./msquic_test -server -cert_hash:<...>\n"
        "  ./msquic_test -server -cert_file:<...> -key_file:<...> [-password:<...>]\n"
        );
}
    ...
    )
{
    ...
    SendBuffer->Length = strlen(Reply);
    memcpy(SendBuffer->Buffer, Reply, SendBuffer->Length);
...
{
    static uint64_t TotalRecvLength = 0;
...
{
    QUIC_SEND_FLAGS Flag = QUIC_SEND_FLAG_NONE;
    static uint64_t TotalSentLength = 0;
    UNREFERENCED_PARAMETER(Context);
    ...
    case QUIC_STREAM_EVENT_SEND_COMPLETE:
        ...
        if (TotalSentLength) {
            Flag = QUIC_SEND_FLAG_NONE;
            if (TotalSentLength == TotalLength - SendBufferLength)
                Flag = QUIC_SEND_FLAG_FIN;
        }
        ...
        if (QUIC_FAILED(Status = MsQuic->StreamSend(Stream, SendBuffer, 1, Flag, SendBuffer))) {
            printf("StreamSend failed, 0x%x!\n", Status);
            free(SendBufferRaw);
            break;
        }
